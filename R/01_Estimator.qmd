...
print("Total world insects estimated tree richness")
...
print("Total world insects described trees")
...
# Approach 2: Calculate insect richness worldwide using the Chao estimator for Microgastrinae and for ACG malaise insects
...
## Step 3 Estimate using iChao1 total number of insects in ACG

```{r}
## Total number of insecta bins in ACG core malaise -- ran the Chao estimator before because it takes a long time to render 

#insc_acg_estimate <- ChaoSpecies(insc$Frequency,"abundance",k=10,conf=0.95)

insc_acg_estimate <-readRDS("../Data/insecta_acg_chao.rds")

n_insects_core <- insc_acg_estimate$Species_table[estimator,]
n_insects_core
```

```{r}

## Calculate total number of insects in ACG
total_insec_acg <- MOVERR(theta1 = n_insects_core['Estimate'], ci1 = c(n_insects_core['95%Lower'], n_insects_core['95%Upper']), theta0 = proportion_core, ci0 = CI_proportion_core)

total_insec_acg$estimate

total_insec_acg$conf.int
```
...
print("Total insects estimated tree richness ")

total_insect_world <- (TW*total_insec_acg$estimate)/TA 
total_insect_world

CI_total_insect_world <- (TW*total_insec_acg$conf.int)/TA 

CI_total_insect_world

print("Total insects described tree richness")


total_insect_world <- (TW_d*total_insec_acg$estimate)/TA 
total_insect_world

CI_total_insect_world <- (TW_d*total_insec_acg$conf.int)/TA 

CI_total_insect_world
```

## Step 4. Estimating global insect richness – based saturniidae moths

```{r}

##### Total insects 

total_insect_world <- (SW*total_insec_acg$estimate)/SA 
total_insect_world

CI_total_insect_world <- (SW*total_insec_acg$conf.int)/SA 
